...
def test_real_analysis_report_keeps_known_public_real_photo_as_real() -> None:
    case_id = "pytest-real-analysis-public-real-photo-001"
    create_response = client.post(
        "/cases",
        json={
            "id": case_id,
    ...
    assert response.status_code == 200
    body = response.json()
    attribution = body["vision_evidence_models"]["vision_generator_attribution"]
    assert attribution["top_candidate"] == "real"
    assert attribution["candidate_ranking"][0]["label"] == "real"
    assert "图像来源研判最高候选：真实照片" in body["report_markdown"]
    assert "图像来源研判最高候选：其他生成模型" not in body["report_markdown"]
